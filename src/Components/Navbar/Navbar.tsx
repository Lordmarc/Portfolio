...
const Navbar = () => {
  ...
  return (
    <div className="bg-white flex items-center justify-between px-6 py-1 drop-shadow-md">
      ...
      >
        <ul className="flex flex-col items-center space-y-4 md:flex-row md:space-x-6 md:space-y-0">
          {menuItems.map((items) => (
            <li key={items.path} className="group w-full text-center">
              <Link className="link group-hover:w-full block" onClick={handleMenu} to={items.path}>{items.name}</Link>
            </li>
          ))}
        </ul>
      </div>
